...
public class Application implements Callable<Integer> {
    ...
    @Option(names = { "-cd",
            "--concreteDriven" }, description = "Enable concrete driven execution (default: ${DEFAULT-VALUE})", defaultValue = "false", paramLabel = "<true|false>")
    private boolean concreteDriven;

    @Option(names = { "-s",
            "--strategy" }, description = "One or multiple of the available search strategies (default: ${DEFAULT-VALUE})", defaultValue = "DFS", split = ",", arity = "1..*", converter = SearchStrategyValidator.class, paramLabel = "<name>")
    private List<String> searchStrategies;

    @Option(names = { "-hu",
            "--heuristic" }, description = "One or multiple of the available search heuristics to use for probabilistic search (default: ${DEFAULT-VALUE})", defaultValue = "Uniform", split = ",", arity = "1..*", converter = SearchHeuristicValidator.class, paramLabel = "<name>")
